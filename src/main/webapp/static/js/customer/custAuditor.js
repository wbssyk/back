...
$(function(){
    ...
    $listGrid.bootstrapTable({
        ...
        columns: [{
            ...
            cellStyle:formatTableUnit,
            formatter:function(idx,row){
                if(row.browserDescribe=='undefined') {
                    return '';
                }else{
                    return row.browserDescribe;
                }
            }
        },{
            field: 'createTime',
         ...
         formatter: function(idx,row){
             ...
             if(roleType==3||roleType==2){
                 str = [
                     //'<i title="审核" style="color:#0079FE;" class="js_details glyphicon glyphicon-pencil"></i>',
    ...
    $listGrid.on("click",'.delOpt',function (){
        ...
        function deleteFor() {
            ...
            $("#ajaxDele",window.parent.document).find('#saveBtn').unbind('click').bind("click",function (e) {
                //e.stopPropagation();
