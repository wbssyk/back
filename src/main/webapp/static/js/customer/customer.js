...
$(function(){
    ...
    $listGrid.bootstrapTable({
        ...
        columns: [{
            ...
            formatter: function(idx,row){
                if(row.browserType==1){
                    return 'windows';
                }else if(row.browserType==2){
                    return 'mac';
                }
            }
        },{
    ...
    function queryParams(params) {
        var temp = {   //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
            ...
            browserName:$(".js_accountNumber").val(),
    ...
    $(".customerAdd").click(function(){
        ...
        _iframe.customerAds('customer/addCustomer.html');
        setTimeout(function(){
            $("#customerAds .popBoxCust .settingBox",window.parent.document).find('.popTitle').text('上传控件');
            $(".roleList",window.parent.document).val('1');
            $(".roleType",window.parent.document).val('1');
            $(".roleFile",window.parent.document).val('1');
            flag = 1;
            list = [];
            var filePicker = $("#popBoxCust #filePicker",window.parent.document);
    ...
    function webUploadrs(filePicker) {
        var $list = $('.uploaders #fileList',window.parent.document),
        ...
        uploader.on( 'uploadSuccess', function( file,response) {
            //$(".pst",window.parent.document).hide();
            if(response.result == '28'){
                modelTxt(response.message);
                $("#customerAds",window.parent.document).find(".js_cancel").trigger('click');
                return false;
            }
        ...
        //新建用户取消按钮
        $("#customerAds",window.parent.document).on("click",".js_cancel",function () {
            if(uploader){
                uploader.destroy();
            }
            hideUser();
        ...
        //绑定提交事件
        $("#customerAds",window.parent.document).on("click",'#btn',function () {
    ...
    //保存 存库
    $("#customerAds",window.parent.document).on("click",".submitUserForm",function(){
        if(list.length==0){
            if(flag==1){
                $("#fileList",window.parent.document).append('<p style="color: red;" class="flagPs">请选择需要上传的文件!</p>');
                flag++;
            }
        ...
        for(var i=0;i<list.length;i++){
            ...
            data.browserName = list[i].fileName;
            data.url = list[i].filePath;
            data.id = list[i].id;
            data.version = list[i].version;
            data.md5 = list[i].md5;
        }
        data.browserDescribe = $(".textarea",window.parent.document).val();
